...
fn run_synops_benchmark(cfg: &ScaleConfig) -> (f64, f64, f64, f64) {
    ...
    // Warm up
    for _ in 0..10 { brain.step(4500.0); }
    brain.reset();
    ...
    println!("  SynOPS/W:         {synops_per_watt:.3e} (at {ESTIMATED_CPU_POWER_W}W CPU estimate)");
...
fn main() {
    ...
    let configs = [
        ScaleConfig { name: "Standard (5N/6S)", neurons: 5, connections: 6, steps: 100_000 },
        ScaleConfig { name: "Small (100N)", neurons: 100, connections: 300, steps: 10_000 },
        ScaleConfig { name: "Medium (1kN)", neurons: 1_000, connections: 3_000, steps: 5_000 },
        ScaleConfig { name: "Large (10kN)", neurons: 10_000, connections: 30_000, steps: 1_000 },
        ScaleConfig { name: "XL (40kN)", neurons: 40_960, connections: 122_880, steps: 500 },
    ];
    ...
    println!("  5-neuron:    {:>8} bytes ({:.1} bytes/neuron)", mem5, mem5 as f64 / 5.0);
    println!("  1k-neuron:   {:>8} bytes ({:.1} bytes/neuron)", mem1k, mem1k as f64 / 1000.0);
    println!("  40k-neuron:  {:>8} bytes ({:.1} bytes/neuron, {:.2} MB)", mem40k, mem40k as f64 / 40960.0, mem40k as f64 / 1_048_576.0);
    ...
    println!("║  Cricket (5N)     │ {:>9.2e}  │ ~15W    │ {:>9.2e}  │ {:>7.0}    ║",
             results[0].1, results[0].2, mem5 as f64 / 5.0);
    println!("║  Cricket (40kN)   │ {:>9.2e}  │ ~15W    │ {:>9.2e}  │ {:>7.0}    ║",
             best.1, best.2, mem40k as f64 / 40960.0);
