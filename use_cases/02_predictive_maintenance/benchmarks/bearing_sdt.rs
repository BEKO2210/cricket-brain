...
//! SDT benchmark for bearing fault detection.
//! Green & Swets (1966) — d', AUC, TPR, FPR with Wilson 95% CI.
//! Date: 2026-04-10
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  Bearing SDT Benchmark — Green & Swets (1966)              ║");
    println!("║  Date: 2026-04-10 | {TRIALS} trials/class | {SIGNAL_LEN} steps    ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");

    println!("  {:<35} {:>5}  {:>5}  {:>6}  {}", "Condition", "TPR", "FPR", "d'", "Rating");
    println!("  {:─>35} {:─>5}  {:─>5}  {:─>6}  {:─>9}", "", "", "", "", "");

    run_condition("Outer Race vs Normal", FaultType::OuterRace,
        vibration_signal::outer_race_fault, vibration_signal::normal_vibration);
    run_condition("Inner Race vs Normal", FaultType::InnerRace,
        vibration_signal::inner_race_fault, vibration_signal::normal_vibration);
    run_condition("Ball Defect vs Normal", FaultType::BallDefect,
        vibration_signal::ball_fault, vibration_signal::normal_vibration);
    run_condition("Normal vs Outer Race", FaultType::Normal,
        vibration_signal::normal_vibration, vibration_signal::outer_race_fault);
